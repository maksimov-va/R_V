...
public:
	...
	unsigned int default_value;// Значение по умолчанию
	const char* available_values; //Список допустимых значений и их названий, формат: "0 A;1 B;3 C"
	unsigned int invalid_available = PAR_ALWAYS_VALID; //Доступность состояния "не достоверный
	...
	// Унаследовано через par_enum_read_only_t
	/*!
	...
	*/
	virtual void set_value_from(const par_t* par) override//нет
	{
	...
	*/
	virtual void save() override
	{
		...
		out.seekp(25);							//шаг 12 байт     первые 24 байт занимают значения классов par_list_t, par_enum_t
	...
	virtual void load() override
	{
		...
		if (out) {
			out.seekg(25);						//шаг 12 байт     первые 24 байт занимают значения классов par_list_t, par_enum_t
			out.read((char*)debug_valid, sizeof(bool));
			out.read((char*)debug_value, sizeof(unsigned int));
			out.close();
		}
	}
	virtual void link_with(par_t* par) override//нет
	{
	}
	virtual void unlink_from(par_t* par) override//нет
	{
	}
	/*!
	...
	virtual unsigned int get_min_value() const override //Не понимаю чте именно необходимо вывести
	{
		return 0;
	}
	virtual unsigned int get_max_value() const override//Не понимаю чте именно необходимо вывести
	{
		return 0;
	}
};
