...
class realise_par_bool_read_only_t :public par_bool_read_only_t {
public:
	...
	realise_par_bool_read_only_t(const unsigned char list_id, const char* text_id, const char* name, const bool default_value = false) {
	...
	*/
	virtual void set_value_from(const par_t* par) override
	{
		if (typeid(*par).name() == typeid(*this).name()) {
			const realise_par_bool_read_only_t* a = dynamic_cast<const realise_par_bool_read_only_t*>(par);
			*this = *a;
		}
	}
	/*!
	...
	virtual void save() override
	{
		...
		out.seekp(83);												//шаг 12 байт     первые 82 байт занимают значения классов par_list_t, par_enum_t,  par_enum_read_only_t, par_double_t, par_double_read_only_t
	...
	virtual void load() override
	{
		...
		if (out) {
			out.seekg(83);											//шаг 12 байт     первые 82 байт занимают значения классов par_list_t, par_enum_t,  par_enum_read_only_t, par_double_t, par_double_read_only_t
...
par_bool_read_only_t* __stdcall create_par_bool_read_only(const unsigned char list_id, const char* text_id, const char* name, const bool default_value) {
	return new realise_par_bool_read_only_t(list_id, text_id, name, default_value);
}
void __stdcall destroy_par_bool_read_only(par_bool_read_only_t* par) {
	delete par;
}